...
export function InputOTP({ length = 6, value = '', onChange, className = '', size = 'md', disabled = false, autoFocus = true }) {
    ...
    const handleKeyDown = (index, e) => {
        ...
        if (e.key === 'Backspace') {
            if (otp[index]) {
                ...
                newOtp[index] = '';
                setOtp(newOtp);
                onChange === null || onChange === void 0 ? void 0 : onChange(newOtp.join(''));
            }
            else if (index > 0) {
        ...
        if (e.key === 'v' && (e.ctrlKey || e.metaKey)) {
            ...
            navigator.clipboard.readText().then(text => {
                ...
                setOtp(newOtp);
                onChange === null || onChange === void 0 ? void 0 : onChange(digits);
    ...
    return (_jsx("div", { className: `flex items-center ${sizeClasses[size].gap} ${className}`, children: groups.map((group, groupIndex) => (_jsxs(React.Fragment, { children: [_jsx("div", { className: `flex ${sizeClasses[size].gap}`, children: group.map((index) => (_jsx("input", { ref: (el) => {
                            inputRefs.current[index] = el;
                        }, type: "text", inputMode: "numeric", pattern: "[0-9]*", maxLength: 1, value: otp[index], onChange: (e) => handleChange(index, e.target.value), onKeyDown: (e) => handleKeyDown(index, e), onFocus: () => handleFocus(index), disabled: disabled, className: `
                ...
                ` }, index))) }), groupIndex < groups.length - 1 && (_jsx("div", { className: `text-foreground font-semibold text-sm sm:text-lg ${sizeClasses[size].separator}`, children: "-" }))] }, groupIndex))) }));
}
